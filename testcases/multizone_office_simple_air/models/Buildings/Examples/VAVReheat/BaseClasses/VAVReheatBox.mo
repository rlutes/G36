...
model VAVReheatBox "Supply box of a VAV system with a hot water reheat coil"
  ...
  parameter Modelica.SIunits.MassFlowRate mCooAir_flow_nominal
    "Nominal air mass flow rate from cooling sizing calculations";
  parameter Modelica.SIunits.MassFlowRate mHeaAir_flow_nominal
  ...
  parameter Modelica.SIunits.Temperature THeaWatInl_nominal(start=55 + 273.15,
      displayUnit="degC") "Reheat coil nominal inlet water temperature";
  parameter Modelica.SIunits.Temperature THeaWatOut_nominal(start=
        THeaWatInl_nominal - 10, displayUnit="degC")
    "Reheat coil nominal outlet water temperature";
  parameter Modelica.SIunits.Temperature THeaAirInl_nominal(start=12 + 273.15,
      displayUnit="degC")
    "Inlet air nominal temperature into VAV box during heating";
  parameter Modelica.SIunits.Temperature THeaAirDis_nominal(start=28 + 273.15,
      displayUnit="degC")
  ...
  Modelica.Blocks.Interfaces.RealOutput y_val_actual(final unit="1")
    "Actual reheat valve position" annotation (Placement(transformation(extent=
            {{100,-90},{120,-70}}), iconTransformation(extent={{100,-50},{120,
            -30}})));
protected
  constant Modelica.SIunits.SpecificHeatCapacity cpAir=Buildings.Utilities.Psychrometrics.Constants.cpAir
...
equation
  ...
  connect(val.y_actual, y_val_actual) annotation (Line(points={{-55,7},{-40,7},
          {-40,-86},{92,-86},{92,-80},{110,-80}},   color={0,0,127}));
  annotation (Icon(
    graphics={
        Rectangle(
